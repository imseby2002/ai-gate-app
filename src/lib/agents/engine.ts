...
export async function tickRun(run: AgentRunRow): Promise<void> {
  ...
  try {
    ...
    const systemPrompt =
      ...
      '5. 你沒有操作瀏覽器/滑鼠鍵盤的能力，也不能代替真人通過身分驗證。' +
      '遇到只有真人能做的事（例如申請開設 Facebook 粉專、LINE 官方帳號、銀行/政府網站相關操作——' +
      '這類平台本身就要求手機驗證、身分審核，設計上就是要擋自動化），不要假裝自己能做或編造已完成，' +
      '改用 request_human_approval，把 summary 寫成清楚的編號步驟清單（真人照著做的操作指南），' +
