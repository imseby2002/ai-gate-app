...
import {
  ...
  ChevronDown, ChevronUp, BedDouble, MapPin, Clock, Users,
  Globe, Plus, HelpCircle, ExternalLink,
} from 'lucide-react'
...
export default function BnbProfilePage() {
  ...
  const PLATFORMS = [
    ...
    { id: 'other',       label: t('profile.import.platformOther'), color: 'bg-gray-500', hint: '' },
  ]
  const UNIT_LABELS: Record<string, string> = {
    per_trip: t('profile.unitLabels.per_trip'), per_stay: t('profile.unitLabels.per_stay'),
    per_night: t('profile.unitLabels.per_night'), per_person: t('profile.unitLabels.per_person'),
  }
  const addonName = (svc: { id: string; name: string }) => t.has(`profile.addons.names.${svc.id}`) ? t(`profile.addons.names.${svc.id}`) : svc.name
  ...
  async function startImportParse() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setImportError(d.error ?? t('profile.import.parseFailed'))
        setImportStep('input')
        return
      }
      ...
      setImportError(t('profile.import.networkError'))
      setImportStep('input')
    }
  }
  ...
  async function applyImport() {
    ...
    const rooms = parsed.rooms?.length ? parsed.rooms : [{ name: parsed.name || t('profile.import.defaultRoom'), description: '', max_guests: 2, base_price: null }]
    ...
    <div className="p-4 md:p-6 pb-16 max-w-2xl space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <button onClick={save} disabled={saving}
          ...
          {saved ? t('profile.saved') : t('detail.save')}
        </button>
      </div>
          ...
          <div className="border-t px-4 sm:px-5 pb-5 pt-4 space-y-4">
            {importStep === 'done' ? (
              <div className="text-center space-y-3 py-4">
                ...
                <p className="text-sm font-semibold text-gray-800">{t('profile.import.appliedTitle')}</p>
                <p className="text-xs text-gray-500">{t('profile.import.appliedHint')}</p>
                <button onClick={() => { setImportOpen(false); setImportStep('input'); setParsed(null) }}
                  className="text-xs text-indigo-600 underline">{t('profile.import.close')}</button>
              </div>
              ...
              <div className="space-y-3">
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2.5 flex items-center gap-2 text-xs text-emerald-800">
                  ...
                  {t('profile.import.parsedBanner')}
                </div>
                <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-xs text-gray-700">
                  <div className="font-semibold text-sm text-gray-900">{parsed.name || t('profile.import.noName')}</div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5 pt-1">
                    ...
                    {parsed.check_in_time && <div className="flex items-center gap-1"><Clock className="h-3 w-3 text-gray-400" />{t('profile.import.checkIn')} {parsed.check_in_time}</div>}
                    {parsed.check_out_time && <div className="flex items-center gap-1"><Clock className="h-3 w-3 text-gray-400" />{t('profile.import.checkOut')} {parsed.check_out_time}</div>}
                  </div>
                  ...
                  {parsed.rooms.length > 0 && (
                    <div className="pt-1">
                      <div className="flex items-center gap-1 text-[10px] font-semibold text-gray-500 mb-1"><BedDouble className="h-3 w-3" />{t('profile.import.roomsFound', { count: parsed.rooms.length })}</div>
                      {parsed.rooms.slice(0, 3).map((r, i) => (
                        <div key={i} className="flex items-center justify-between py-0.5">
                          <span>{r.name}</span>
                          <span className="text-gray-500 flex items-center gap-0.5"><Users className="h-2.5 w-2.5" />{t('profile.import.personsUnit', { count: r.max_guests })}{r.base_price ? ` · NT$${r.base_price}` : ''}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  {importImages.length > 0 && (
                    <div className="pt-2">
                      <div className="text-[10px] font-semibold text-gray-500 mb-1.5">{t('profile.import.imagesFound', { count: importImages.length })}</div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setImportStep('input')}
                    className="flex-1 py-2 rounded-lg border text-xs text-gray-600 hover:bg-gray-50">{t('profile.import.reenter')}</button>
              ...
              <div className="space-y-4">
                ...
                {importMode === 'url' ? (
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-gray-400">{t('profile.import.urlHint')}</p>
                  </div>
                ) : (
                  <div className="space-y-1">
                    <textarea value={importText} onChange={e => setImportText(e.target.value)}
                      rows={5} placeholder={t('profile.import.textPlaceholder')}
                ...
                {importError && (
                  <div className="bg-rose-50 border border-rose-200 rounded-lg px-3 py-2.5 flex items-start gap-2 text-xs text-rose-700">
                    ...
                    <div>
                      <p>{importError}</p>
                      {importFetchFailed && <p className="mt-0.5 text-rose-600">{t('profile.import.fetchFailedHint')}</p>}
                    </div>
                  </div>
                )}
                ...
                <button type="button" onClick={startImportParse}
                  ...
                  {importStep === 'parsing'
                    ? <><Loader2 className="h-4 w-4 animate-spin" /> {t('profile.import.parsing')}</>
                    : <><Download className="h-4 w-4" /> {t('profile.import.start')}</>}
                </button>
              </div>
            )}
          </div>
      ...
      <section className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 sm:p-5 space-y-3">
        <h2 className="text-sm font-semibold text-indigo-700 flex items-center gap-1.5"><Link2 className="h-4 w-4" /> {t('profile.link.title')}</h2>
        <div className="flex gap-2">
          <div className="flex-1 space-y-1">
            <label className="text-xs text-indigo-600">{t('profile.link.slugLabel')}</label>
      ...
      <section className="bg-white rounded-xl border p-4 sm:p-5 space-y-4">
        <h2 className="text-sm font-semibold text-gray-700">{t('profile.basicInfo')}</h2>
        <div className="space-y-1">
          <label className="text-xs font-medium text-gray-600">{t('profile.bnbName')} <span className="text-red-500">*</span></label>
          <input value={form.name} onChange={e => set('name', e.target.value)}
            placeholder={t('profile.bnbNamePlaceholder')}
            className="w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-300" />
        </div>
        <div className="space-y-1">
          <label className="text-xs font-medium text-gray-600">{t('profile.bnbDesc')}</label>
          <textarea value={form.description} onChange={e => set('description', e.target.value)}
            rows={3} placeholder={t('profile.bnbDescPlaceholder')}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">{t('profile.city')}</label>
            <input value={form.city} onChange={e => set('city', e.target.value)}
              placeholder={t('profile.cityPlaceholder')}
              className="w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-300" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">{t('profile.address')}</label>
      ...
      {/* 聯絡方式 */}
      <section className="bg-white rounded-xl border p-4 sm:p-5 space-y-4">
        <h2 className="text-sm font-semibold text-gray-700">{t('profile.contact')}</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            { key: 'phone',   label: t('profile.phone'),   placeholder: '0912-345-678' },
            { key: 'email',   label: t('profile.email'),   placeholder: 'info@example.com' },
            { key: 'website', label: t('profile.website'), placeholder: 'https://...' },
      ...
      {/* 入住規則 */}
      <section className="bg-white rounded-xl border p-4 sm:p-5 space-y-4">
        <h2 className="text-sm font-semibold text-gray-700">{t('profile.rules')}</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">{t('profile.checkInTime')}</label>
            <input value={form.check_in_time} onChange={e => set('check_in_time', e.target.value)}
              placeholder="15:00"
              className="w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-300" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">{t('profile.checkOutTime')}</label>
            <input value={form.check_out_time} onChange={e => set('check_out_time', e.target.value)}
              placeholder="11:00"
              className="w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-300" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">{t('profile.minNights')}</label>
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium text-gray-600">{t('profile.houseRules')}</label>
          <textarea value={form.house_rules} onChange={e => set('house_rules', e.target.value)}
            rows={4} placeholder={t('profile.houseRulesPlaceholder')}
            className="w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-none" />
        </div>
      </section>
      ...
      {/* 早餐設定 */}
      <section className="bg-white rounded-xl border p-4 sm:p-5 space-y-4">
        <h2 className="text-sm font-semibold text-gray-700">{t('profile.breakfast.title')}</h2>

        <div className="flex gap-2">
          {(['none','included','optional'] as const).map(bt => (
            <button key={bt} type="button" onClick={() => setBreakfast({ type: bt })}
              className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors
                ${breakfast.type === bt
                  ? 'bg-indigo-600 text-white border-indigo-600'
                  : 'bg-white text-gray-600 border-gray-300 hover:border-indigo-400'}`}>
              {bt === 'none' ? t('profile.breakfast.none') : bt === 'included' ? t('profile.breakfast.included') : t('profile.breakfast.optional')}
            </button>
          ))}
        </div>
        ...
        {breakfast.type !== 'none' && (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-xs font-medium text-gray-600">{t('profile.breakfast.start')}</label>
                <input value={breakfast.start_time} onChange={e => setBreakfast({ start_time: e.target.value })}
                  placeholder="07:30"
                  className="w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-300" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-gray-600">{t('profile.breakfast.end')}</label>
            ...
            {breakfast.type === 'optional' && (
              <div className="space-y-1">
                <label className="text-xs font-medium text-gray-600">{t('profile.breakfast.price')}</label>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400">{t('profile.breakfast.perPersonUnit')}</span>
                </div>
              </div>
            )}
            <div className="space-y-1">
              <label className="text-xs font-medium text-gray-600">{t('profile.breakfast.desc')}</label>
              <textarea value={breakfast.description}
                onChange={e => setBreakfast({ description: e.target.value })}
                rows={2} placeholder={t('profile.breakfast.descPlaceholder')}
      ...
      <section className="bg-white rounded-xl border p-4 sm:p-5 space-y-3">
        <h2 className="text-sm font-semibold text-gray-700">{t('profile.addons.title')}</h2>
        <p className="text-xs text-gray-400">{t('profile.addons.hint')}</p>
          ...
          {addon_services.map(svc => (
            <div key={svc.id} className="py-3 space-y-2">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-gray-700">{addonName(svc)}</span>
              </div>
              {svc.enabled && (
                <div className="ml-[52px] grid grid-cols-2 sm:grid-cols-3 gap-2">
                  <div className="space-y-1">
                    <label className="text-[11px] text-gray-500">{t('profile.addons.price')}</label>
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-gray-400 shrink-0">{t('profile.addons.priceUnit')}</span>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[11px] text-gray-500">{t('profile.addons.billingUnit')}</label>
                  ...
                  <div className="space-y-1 col-span-2 sm:col-span-1">
                    <label className="text-[11px] text-gray-500">{t('profile.addons.note')}</label>
                    <input value={svc.note} onChange={e => updateAddon(svc.id, { note: e.target.value })}
                      placeholder={t('profile.addons.notePlaceholder')}
                      className="w-full text-sm border rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-300" />
                  </div>
                </div>
              )}
      ...
      {/* 官網編輯器入口 */}
      <section className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-xl p-5 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-semibold text-gray-800">{t('nav.website')}</h2>
              <p className="text-xs text-gray-500">{t('profile.web.subtitle')}</p>
            </div>
          </div>
          {form.slug && (
            <a href={`/book/${form.slug}`} target="_blank" rel="noreferrer"
              className="flex items-center gap-1 text-xs text-indigo-500 hover:underline">
              <ExternalLink className="h-3 w-3" /> {t('profile.web.openSite')}
            </a>
          )}
        </div>
        <p className="text-xs text-gray-500">{t('profile.web.desc')}</p>
        <a href="/booking/website"
          ...
          <Globe className="h-4 w-4" /> {t('profile.web.enter')}
        </a>
      </section>

      {/* 民宿照片 */}
      <section className="bg-white rounded-xl border p-4 sm:p-5 space-y-3">
        <h2 className="text-sm font-semibold text-gray-700">{t('profile.photos.title')}</h2>
        <p className="text-xs text-gray-400">{t('profile.photos.hint')}</p>
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
          {form.images.map((url, i) => (
            <div key={url} className="relative aspect-square rounded-xl overflow-hidden border group">
              ...
              {i === 0 ? (
                <span className="absolute top-1 left-1 text-[10px] bg-indigo-600 text-white px-1.5 py-0.5 rounded-full">{t('profile.photos.main')}</span>
              ) : (
      ...
      {dirty && (
        <div className="fixed bottom-0 left-0 right-0 z-40 border-t bg-white/95 backdrop-blur px-4 py-3 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
          <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
            <span className="text-sm text-amber-600">{t('profile.unsaved')}</span>
            <button onClick={save} disabled={saving}
