          ...
          <div className="space-y-2">
            {checkingInToday.map(b => (
              <div key={b.id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-100">
                <div>
                  <div className="font-medium text-sm text-gray-900">{b.guest_name || '(未知旅客)'}</div>
                  <div className="text-xs text-gray-500">{b.properties?.name} · 退房 {b.check_out}</div>
                </div>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${PLATFORM_COLORS[b.platform] ?? 'bg-gray-100 text-gray-600'}`}>
          ...
          <div className="divide-y">
            {upcoming.slice(0, 10).map(b => (
              <div key={b.id} className="flex items-center justify-between py-2.5">
                <div>
                  <div className="font-medium text-sm text-gray-900">{b.guest_name || '(未知旅客)'}</div>
                  <div className="text-xs text-gray-500">{b.properties?.name} · {b.check_in} → {b.check_out}</div>
                </div>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${PLATFORM_COLORS[b.platform] ?? 'bg-gray-100 text-gray-600'}`}>
                  {PLATFORM_NAMES[b.platform] ?? b.platform}
                </span>
