...
export async function GET(req: NextRequest) {
  ...
  let q = supabase.from('ical_settings').select('*, properties(name)').eq('user_id', ctx.ownerId).order('created_at')
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase
    .from('ical_settings')
    .insert({ user_id: ctx.ownerId, property_id, platform, platform_name, ical_url, sync_interval })
    .select().single()
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await supabase.from('ical_settings').delete().eq('id', id).eq('user_id', ctx.ownerId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
