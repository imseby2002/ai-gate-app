...
type Flag = { dept: string; level: 'urgent' | 'warn' | 'info'; text: string }
...
export async function GET() {
  ...
  const [{ data: orders }, { data: equip }] = await Promise.all([
    admin.from('repair_orders').select('status, reported_at, cost').eq('owner_id', ownerId),
    admin.from('repair_equipment').select('name, warranty_until, status').eq('owner_id', ownerId),
  ])
  ...
  // ── 外務／法遵：文件到期 ──
  const { data: docs } = await admin.from('affair_documents').select('title, doc_type, expiry_date, status, remind_days_before').eq('owner_id', ownerId).not('expiry_date', 'is', null)
  const expiringDocs = (docs ?? [])
    .filter(d => d.status !== 'archived' && d.expiry_date)
    .map(d => ({ title: d.title, doc_type: d.doc_type, expiry_date: d.expiry_date, days: daysTo(d.expiry_date as string) }))
    .filter(d => d.days <= 30)
  ...
  // ── 人事 ──
  const monthStart = new Date(); monthStart.setDate(1); const monthStr = monthStart.toISOString().slice(0, 10)
  ...
  // 紅旗排序：urgent > warn > info
  const order = { urgent: 0, warn: 1, info: 2 }
  flags.sort((a, b) => order[a.level] - order[b.level])

  return NextResponse.json({ period, finance, repair, affairs, hr, audit, flags })
}
