// 行銷中心「實體行銷」：沿用 mkt_offline 資料（與辦公室 /mkt 同一份），改以行銷模組權限存取。
...
async function auth() {
  ...
  if (!isSuperAdmin && profile.company_id) {
    const { data: m } = await admin.from('company_members')
      .select('member_id').eq('company_id', profile.company_id).eq('role', 'owner').eq('status', 'active').maybeSingle()
    if (m?.member_id) ownerId = m.member_id
  }
  return { admin, ownerId }
}

export async function GET(req: NextRequest) {
  const a = await auth(); if (!a) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  const sp = new URL(req.url).searchParams
  let q = a.admin.from('mkt_offline')
    .select('id, type, title, store, status, start_date, end_date, budget, counterparty, photo_url, note, created_at')
    .eq('owner_id', a.ownerId)
...
export async function POST(req: NextRequest) {
  ...
  if (!s(b.title)) return NextResponse.json({ error: '標題必填' }, { status: 400 })
  const { data, error } = await a.admin.from('mkt_offline').insert({
    owner_id: a.ownerId,
    type: TYPES.includes(s(b.type)) ? s(b.type) : 'material',
    title: s(b.title), store: s(b.store),
...
export async function PATCH(req: NextRequest) {
  ...
  const { error } = await a.admin.from('mkt_offline').update(upd).eq('id', id).eq('owner_id', a.ownerId)
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: 'id required' }, { status: 400 })
  const { error } = await a.admin.from('mkt_offline').delete().eq('id', s(id)).eq('owner_id', a.ownerId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json({ ok: true })
}
