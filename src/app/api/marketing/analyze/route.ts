...
7. 內容差異化建議：我方如何在內容上做出差異`,
  ...
  review_insights: `**口碑體檢（顧客評論分析 → 改善方向）**
請把上方蒐集到的評論「當作本公司／本店自己的顧客評論」來體檢（若資料明顯屬於競品，請於開頭一句提醒）：
1. 整體印象：正負評大致比例、整體評分與近期變化訊號（依可得資料推估）
2. 正評重點：顧客最滿意的 3-5 點（哪些要繼續保持、可拿來當行銷賣點）
3. 負評主題分類：把抱怨歸類（服務／產品品質／價格／環境設施／等候時間／流程或制度／溝通態度…），每類標出出現頻率（高/中/低）與代表性原句摘要
4. 改善方向（依「影響 × 急迫」排優先序）：列 4-6 項，每項給**具體可執行做法**，不要空泛口號
5. 部門指派建議：每個改善項標註應由哪個單位跟進（如門市／客服／產品／行政）
6. 公關回覆方向：面對負評時的回覆話術基調與 1 個示範回覆
最後給「口碑健康度」評分（0-100）與一句總結。`,
