...
import { buildIvtXlsx } from '@/lib/inv/ivt'
...
export async function GET(req: NextRequest) {
  ...
  // IVT 匯入檔：kind=ivt-count（實盤）/ ivt-order（訂貨量）&id=
  if (kind === 'ivt-count' || kind === 'ivt-order') {
    const id = s(sp.get('id'))
    if (!id) return NextResponse.json({ error: 'id required' }, { status: 400 })
    const out = await buildIvtXlsx(supabase, user.id, id, kind)
    if (!out) return NextResponse.json({ error: 'not found' }, { status: 404 })
    return xlsxResponse(out.buf, out.filename)
  }
