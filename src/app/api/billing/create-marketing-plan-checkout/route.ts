...
import { MARKETING_PLAN_PACKAGES, type MarketingPlanPackageId } from '@/lib/ecpay/marketing-plans'

// POST /api/billing/create-marketing-plan-checkout
// 客戶自助升級行銷方案：一次性 ECPay 付款（非自動續訂），到期前需自行再次購買延續。
export async function POST(req: NextRequest) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })

  const { packageId } = await req.json() as { packageId: MarketingPlanPackageId }
  const pkg = MARKETING_PLAN_PACKAGES.find(p => p.id === packageId)
  if (!pkg) return NextResponse.json({ error: '無效的方案' }, { status: 400 })

  // 現有方案尚未到期時，禁止購買「較低」方案：付款回調會直接覆蓋訂閱，
  // 等於立刻降級且剩餘天數全部消失。同方案續購（延長）與升級不受限（比照 CS 方案結帳）。
  ...
  if (subActive && (PLAN_RANK[pkg.plan] ?? 0) < (PLAN_RANK[currentSub!.plan] ?? 0)) {
    ...
    return NextResponse.json({
      error: `目前的 ${String(currentSub!.plan).toUpperCase()} 方案尚未到期${endDate ? `（至 ${endDate}）` : ''}，購買較低方案會立即降級並喪失剩餘天數。請於到期後再購買。`,
    }, { status: 400 })
  }
  ...
  const { error: insertErr } = await supabase.from('marketing_plan_purchases').insert({
    user_id: user.id,
    trade_no: tradeNo,
    plan: pkg.plan,
  ...
  const params: Record<string, string> = {
    ...
    OrderResultURL:   `${appUrl}/marketing/plan?upgrade=done`,
    ChoosePayment:    'ALL',
    EncryptType:      '1',
    ClientBackURL:    `${appUrl}/marketing/plan?upgrade=cancel`,
  }
