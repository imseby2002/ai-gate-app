...
const getPanelMeta = (t: (key: string) => string): Record<RequestType, { icon: typeof Wrench; title: string; placeholder: string }> => ({
  feature: { icon: Wrench, title: t('featureTitle'), placeholder: t('featurePlaceholder') },
  feedback: { icon: MessageSquareWarning, title: t('feedbackTitle'), placeholder: t('feedbackPlaceholder') },
})

export function CsSupportPanel() {
  const t = useTranslations('CsSupportPanel')
  const PANEL_META = getPanelMeta(t)
  ...
  const submit = async (type: RequestType) => {
    if (!note.trim()) { setError(t('errNote')); return }
    ...
    try {
      ...
      if (!res.ok) { setError(data.error ?? t('submitFailed')); return }
      setSentType(type)
  ...
  return (
    <div className="bg-card border rounded-xl p-4 space-y-3">
      <div className="font-medium text-sm text-foreground flex items-center gap-2">
        <LifeBuoy className="h-4 w-4 text-primary" />{t('needHelp')}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        <Link href="/cs/settings"
          ...
          <span className="flex-1">{t('helpSetup')}</span>
        ...
        <div className="rounded-lg border bg-muted/30 p-3 space-y-2.5">
          {sentType === open ? (
            <div className="flex items-center gap-2 text-sm text-emerald-600 dark:text-emerald-400">
              ...
              {t('sentPrefix')}
              {priceUsd != null && t('sentPriceNote', { price: priceUsd })}
            </div>
          ) : (
            <>
