...
const getKindLabel = (t: (key: string) => string): Record<PnlLineKind, string> => ({
  revenue: t('kindRevenue'), detail: t('kindDetail'), subtotal: t('kindSubtotal'), compare: t('kindCompare'),
})
...
export default function PnlSchemaEditor({ initial, onClose, onSaved }: {
  ...
  const KIND_LABEL = getKindLabel(t)
  ...
  const save = async () => {
    if (rows.length === 0) { setErr(t('needAtLeastOneLine')); return }
    if (rows.some(r => !r.zh.trim())) { setErr(t('needZhNameForEach')); return }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <div>
          <h3 className="text-sm font-semibold">{t('lineSettingsTitle')}</h3>
          <p className="text-xs text-gray-400">{t('lineSettingsDesc')}</p>
        </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 flex-1">
                <div className="space-y-1">
                  <label className="text-[11px] text-gray-400">{t('zhNameLabel')}</label>
                  <Input value={r.zh} onChange={e => patch(r._rid, { zh: e.target.value })} className="h-8 text-sm" placeholder={t('zhNamePlaceholder')} />
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] text-gray-400">{t('viNameLabel')}</label>
                  <Input value={r.vi} onChange={e => patch(r._rid, { vi: e.target.value })} className="h-8 text-sm" placeholder="tien dien" />
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] text-gray-400">{t('sectionForSumLabel')}</label>
                ...
                <div className="space-y-1">
                  <label className="text-[11px] text-gray-400">{t('typeLabel')}</label>
...
function ComputeEditor({ value, onChange, codeOpts, sections }: {
  ...
  const CodeSel = ({ v, on }: { v: string; on: (c: string) => void }) => (
    <select value={v} onChange={e => on(e.target.value)}
      ...
      <option value="">{t('selectLinePlaceholder')}</option>
  ...
  return (
    <div className="ml-8 rounded-lg border bg-gray-50 p-2.5 space-y-2">
      <div className="flex items-center gap-2 flex-wrap text-xs">
        <span className="text-gray-500">{t('formulaLabel')}</span>
        <select value={value.op} onChange={e => setOp(e.target.value as PnlCompute['op'])}
          className="h-7 rounded-md border bg-background px-2 text-xs">
          <option value="sum">{t('opSumLines')}</option>
          <option value="sumSection">{t('opSumSection')}</option>
          <option value="sub">{t('opSub')}</option>
          <option value="subMany">{t('opSubMany')}</option>
        </select>
      ...
      {value.op === 'sum' && (
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <button onClick={() => onChange({ op: 'sum', codes: [...value.codes, ''] })}
            className="inline-flex items-center gap-0.5 text-xs text-primary hover:underline"><Plus className="h-3 w-3" />{t('addLineShort')}</button>
        </div>
      )}
      ...
      {value.op === 'subMany' && (
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="text-gray-400 text-xs">{t('minusLabel')}</span>
          {value.minus.map((c, i) => (
            <span key={i} className="inline-flex items-center gap-1">
              {i > 0 && <span className="text-gray-400 text-xs">，</span>}
          ...
          <button onClick={() => onChange({ ...value, minus: [...value.minus, ''] })}
            className="inline-flex items-center gap-0.5 text-xs text-primary hover:underline"><Plus className="h-3 w-3" />{t('addItem')}</button>
        </div>
      )}
    </div>
  )
