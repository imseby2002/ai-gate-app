...
export default function PublicBookingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [confirmation, setConfirmation] = useState<{ code: string; total: number | null } | null>(null)
  ...
  async function submit() {
    ...
    try {
      ...
      const d = await res.json()
      if (!res.ok) { alert(d.error ?? '訂房失敗'); return }
      setConfirmation({ code: d.booking.confirmation_code, total: d.booking.total_price })
      setStep('done')
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        {step === 'done' && confirmation ? (
          ...
          <div className="bg-white rounded-2xl border p-8 text-center space-y-4">
            ...
            {confirmation.total && (
              <p className="text-sm text-gray-500">應付金額：<span className="font-semibold text-gray-900">NT$ {fmt(confirmation.total)}</span></p>
            )}
            <p className="text-sm text-gray-500 max-w-sm mx-auto">民宿業者將與您聯繫確認，請留意您的電子郵件。</p>
          ...
          <div className="space-y-4">
            ...
            <button onClick={submit} disabled={submitting || !form.guest_name || !form.guest_email || !form.check_in || !form.check_out}
